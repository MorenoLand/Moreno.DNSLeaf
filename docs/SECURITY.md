...
- Prefer `tls://` or `https://` upstream endpoints for untrusted networks. DNSLeaf strips EDNS Client Subnet options by default and validates upstream IDs and questions before accepting responses.
- Query history can be disabled, retained for a bounded period, or stored with anonymized client prefixes. Administrator changes are recorded in a separate audit history.
...
- Protect `/metrics`, `/api/audit`, and `/api/backup` like the administration panel. Backup archives contain password hashes and private deployment data.
