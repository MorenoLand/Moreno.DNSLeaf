...
- `GET /metrics` is an authenticated Prometheus-compatible metrics endpoint.
...
- `/api/audit` for administrator-only state-change audit entries
- `/api/backup` for an administrator-only ZIP export of configuration, runtime state, and cached blocklists
...
Secure upstream endpoint examples:

```json
{
  "upstream_endpoints": [
    {"url": "tls://cloudflare-dns.com:853"},
    {"url": "https://dns.google/dns-query"}
  ]
}
```

TLS endpoint configuration uses certificate verification and TLS 1.2 or newer. DNSLeaf does not yet perform local DNSSEC chain validation; use a validating upstream when DNSSEC authenticity is required.
